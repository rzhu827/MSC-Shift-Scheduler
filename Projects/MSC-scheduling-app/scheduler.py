...
def process_data(col_ranges): 
  with open('tutor_responses.csv', 'r') as file:
    ...
    # 3D list of 0's and 1's. available_shifts[t][d][s] = 1 means tutor t is available on day d, shift s.
    # t, d, and s are all indices.
    available_shifts = []
    preferred_shifts = []
    tutor_list = []
    for line in reader:
      tutor = parse_tutor(line, col_ranges, header_to_time, hours_to_indices, header, shifts_range, days_range)
      tutor_list.append(tutor)
      available_shifts.append(tutor.available_shifts)
      preferred_shifts.append(tutor.preferred_shifts)
